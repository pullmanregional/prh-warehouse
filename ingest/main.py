...
async def run_pipeline(run_ingest=True, run_transform=True):
    ...
    # venv (prh-warehouse). Specify a venv name so it is reused between runs.
    await shell_op(
        cmd="pipenv install",
...
async def ingest_patients():
    cmd = f'pipenv run python ingest_patients.py -i "{PRW_EPIC_SOURCE_DIR}" --prw "{PRW_CONN}" --prwid "{PRW_ID_CONN}"'
...
async def ingest_encounters():
    cmd = f'pipenv run python ingest_encounters.py -i "{PRW_EPIC_SOURCE_DIR}" --prw "{PRW_CONN}" --prwid "{PRW_ID_CONN}"'
    return await shell_op(
        cmd=cmd,
        cwd=INGEST_CODE_ROOT,
...
async def ingest_notes():
    cmd = f'pipenv run python ingest_notes.py -i "{PRW_EPIC_SOURCE_DIR}" --prw "{PRW_CONN}" --prwid "{PRW_ID_CONN}"'
    return await shell_op(
        cmd=cmd,
        cwd=INGEST_CODE_ROOT,
...
async def ingest_charges():
    cmd = f'pipenv run python ingest_charges.py -i "{PRW_CHARGES_SOURCE_DIR}" --prw "{PRW_CONN}" --prwid "{PRW_ID_CONN}" --rvu-mapping "{PRW_RVU_MAPPING_SOURCE_DIR}"'
    return await shell_op(
        cmd=cmd,
        cwd=INGEST_CODE_ROOT,
...
async def ingest_imaging():
    cmd = f'pipenv run python ingest_imaging.py -i "{PRW_EPIC_SOURCE_DIR}" --prw "{PRW_CONN}"'
    return await shell_op(
        cmd=cmd,
        cwd=INGEST_CODE_ROOT,
...
async def ingest_finance():
    cmd = f'pipenv run python ingest_finance.py -i "{PRW_FINANCE_SOURCE_DIR}" --prw "{PRW_CONN}" --epic-in "{PRW_EPIC_SOURCE_DIR}"'
    return await shell_op(
        cmd=cmd,
        cwd=INGEST_CODE_ROOT,
